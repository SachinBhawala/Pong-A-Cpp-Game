...
simulate_game(Input* input, float dt) {
    ...
    if (current_gamemode == GM_GAMEPLAY) {
        ...
        draw_text("PLAYER:1", -80, 49.f, 0.6, 0xffffff);
        draw_text("PLAYER:2", 52, 49.f, 0.6, 0xffffff);

        // Check for game over condition (score reaches 10)
        if (player_1_score >= 10) {
            draw_text("PLAYER:1 WINS", -58, 10, 1.5, 0xff0000);
            draw_text("PRESS ENTER TO RETURN TO MENU", -52, -10, 0.6, 0xffffff);
            ...
            draw_text("PLAYER:2 WINS", -58, 10, 1.5, 0x0000ff);
            draw_text("PRESS ENTER TO RETURN TO MENU", -52, -10, 0.6, 0xffffff);
            if (pressed(BUTTON_ENTER)) {
                current_gamemode = GM_MENU;
                player_1_score = 0;
            ...
            // Only display the scores when the game is not over
            draw_number(player_1_score, -10, 40, 1.f, 0xff0000);
            draw_number(player_2_score, 10, 40, 1.f, 0xff0000);

            // Rendering
            draw_rect(ball_p_x, ball_p_y, ball_half_size, ball_half_size, 0xffffff);

            draw_rect(80, player_1_p, player_half_size_x, player_half_size_y, 0x008000);
        ...
        if (hot_button == 0) {
            draw_text("SINGLE-PLAYER", -80, 02, 0.9, 0xff0000);
            draw_text("0", -48, -6, 0.9, 0xff0000);
            draw_text("MULTIPLAYER", 20, 02, 0.9, 0xffffff);
        }
        else {
            draw_text("SINGLE-PLAYER", -80, 02, 0.9, 0xffffff);
            draw_text("MULTIPLAYER", 20, 02, 0.9, 0xff0000);
            draw_text("0", 48, -6, 0.9, 0xff0000);
        }

        draw_text("PONG GAME", -50, 35, 2, 0x000000);
        draw_text("MADE BY:", -32, -38, .38, 0xffffff);
        draw_text("SACHIN BHAWALA", -10, -37, .50, 0xffffff);
    }
}
